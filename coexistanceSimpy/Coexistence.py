        ...
        while self.back_off_time > -1:
            try:
                ...
                if self.first_interrupt and start is not None:
                    ...
                    log(self, f"Backoff time reduced by {(self.env.now - start)}, new backoff: {self.back_off_time}")
                    self.first_interrupt = False
...
class Gnb:
        ...
        while self.back_off_time > -1:
            try:
                ...
                if self.first_interrupt and start is not None and self.waiting_backoff is True:
                    ...
                    # slots_waited = already_waited / self.config_nr.observation_slot_duration
                    # log(self, f"already waited {slots_waited} slots of 9us")
                    # self.back_off_time -= (
                    #         slots_waited * self.config_nr.observation_slot_duration)  # set the Back Off to the remaining one  ## ZMIANA ___ BACKOFF !!!!!!!!!!!!
                    # self.first_interrupt = False
    ...
    def generate_new_back_off_time(self, failed_transmissions_in_row):
        ...
        upper_limit = (
            upper_limit if upper_limit <= self.cw_max else self.cw_max)  # set upper limit to CW Max if is bigger then this parameter
        back_off = random.randint(1, upper_limit)  # draw the back off value
        self.channel.backoffs[back_off][self.channel.n_of_stations] += 1  # store drawn value for future analyzes
...
def run_simulation(
    ...
    # colison probability -- NRU:
    # with open("nru_colision_gap3.csv", mode='a', newline="") as result_file:
    #     result_adder = csv.writer(result_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
    #
    #     colision = (channel.failed_transmissions / (
    #             channel.failed_transmissions + channel.succeeded_transmissions)) * 100
    #
    #     result_adder.writerow(
    #         [seed, number_of_gnb, channel.failed_transmissions, channel.succeeded_transmissions, colision])

    # with open("nru_colision.csv", mode='a', newline="") as result_file:
    #     result_adder = csv.writer(result_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
    #
    ...
    # nru airtime
    # with open("nru_airtime_gap4.csv", mode='a', newline="") as result_file:
    #     result_adder = csv.writer(result_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
    #
    #     channel_occupancy_time = 0
    #     channel_efficiency = 0
    ...
    #
    #     for i in range(1, number_of_gnb + 1):
    #         channel_occupancy_time += channel.airtime_data["Gnb {}".format(i)] + channel.airtime_control[
    #             "Gnb {}".format(i)]
